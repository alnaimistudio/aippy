...
try{
  ...
  for(const d of api.deer){
    d.rig=Object.create(null);
    d.base=Object.create(null);
    const mixer=new THREE.AnimationMixer(d.model);
    const move=mixer.clipAction(locomotion,d.model);
    move.enabled=true;
    move.setLoop(THREE.LoopRepeat,Infinity);
    move.clampWhenFinished=false;
    move.play();
    let deathAction=null;
    if(death){
      deathAction=mixer.clipAction(death,d.model);
    ...
    for(const d of api.deer){
      ...
      if(state==='dead'){
        if(!c.deadPlayed){
          c.deadPlayed=true;
          c.move.stop();
          if(c.death)c.death.reset().fadeIn(.05).play();
        }
      }else{
        if(c.deadPlayed){
          c.deadPlayed=false;
        ...
        c.move.paused=false;
        c.move.timeScale=state==='run'?1.75:.72;
        if(!c.move.isRunning())c.move.play();
      }
      c.mixer.update(step);
    }
  }
  ...
  console.error('[Forest Stalker] anim59 failed',err);
}
